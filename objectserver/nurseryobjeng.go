...
// server. The difference with this object is that on inital write objects
// are written to a nursery (kinda like the existing objects dir) and on
// write they do not have any affect on the hashes.pkl. You can think of
// them as basically handoff objects in this regard. After they are written
// a new replicator will walk the dirs and for each object HEAD the other
// primaries looking for that object. If it finds it there it will rename
// the object to the objects-stable dir. What this does is divide the
...
func (o *nurseryObject) Commit(metadata map[string]string) error {
	...
	go func() {
		...
		InvalidateHash(o.nurseryHashDir) // keeping this for backwards compat
	}()
	return nil
}
...
func (o *nurseryObject) Stabilize() error {
	// TODO later change this so actual stabilze handled by engine
...
func (f *nurseryEngine) New(vars map[string]string, needData bool, asyncWG *sync.WaitGroup) (Object, error) {
	...
	var err error
	sor := &nurseryObject{reclaimAge: f.reclaimAge, reserve: f.reserve, asyncWG: asyncWG}
...
func (f *nurseryEngine) GetNurseryObjects(device string, c chan Object, cancel chan struct{}) {
	defer close(c)
	...
	for _, partition := range partitions {
		/*
			...
			if err != nil {
				f.logger.Error("Skipping invalid file in objects directory", zap.String("worker", "nursery"), zap.String("partitionDir", filepath.Join(objDirPath, partition)))
				continue
			}
				i kinda want to weed these out here but makes interface ugly
				if _, handoff := ring.GetJobNodes(partitioni, dev.Id); handoff {
					continue
				}
		*/
...
func nurseryEngineConstructor(config conf.Config, policy *conf.Policy, flags *flag.FlagSet) (ObjectEngine, error) {
	driveRoot := config.GetDefault("object-replicator", "devices", "/srv/node")
	reserve := config.GetInt("object-replicator", "fallocate_reserve", 0)
	hashPathPrefix, hashPathSuffix, err := conf.GetHashPrefixAndSuffix()
	if err != nil {
		return nil, errors.New("Unable to load hashpath prefix and suffix")
	}
	reclaimAge := int64(config.GetInt("object-replicator", "reclaim_age", int64(common.ONE_WEEK)))
	logLevelString := config.GetDefault("object-replicator", "log_level", "INFO")
	logLevel := zap.NewAtomicLevel()
	logLevel.UnmarshalText([]byte(strings.ToLower(logLevelString)))
	logger, err := srv.SetupLogger("object-replicator", &logLevel, flags)
	if err != nil {
